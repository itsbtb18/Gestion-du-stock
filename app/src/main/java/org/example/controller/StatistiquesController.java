...
public class StatistiquesController implements Initializable {
    ...
    private void loadKPIs(LocalDate debut, LocalDate fin) {
        try {
            ...
            double[] stats = statsService.getSalesStatistics(debut, fin);
            lblChiffreAffaires.setText(String.format("%.2f DZD", stats[1]));
            lblNombreVentes.setText(String.valueOf((int)stats[0]));
            lblPanierMoyen.setText(String.format("%.2f DZD", stats[2]));
    ...
    private void loadTopProductsChart(LocalDate debut, LocalDate fin) {
        ...
        for (Produit produit : topProduits) {
            ...
            series.getData().add(new XYChart.Data<>(
                produit.getNom(),
                Math.max(0, 100 - produit.getQuantiteStock()) 
            ));
        }
